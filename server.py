...
"""CORS configuration: set ALLOW_ORIGINS env to comma list or *"""
# IMPORTANT: the default must be a single string with comma-separated origins (previous manual edit broke this)
RAW_ALLOWED_ORIGINS = os.getenv(
    "ALLOW_ORIGINS",
    "https://ppap-three.vercel.app,https://da5caabcecf3.ngrok-free.app"
)  # e.g. "https://ppap-three.vercel.app,https://foo.bar"
if CORS:
    # Use '*' only if explicitly requested; otherwise let after_request decide per origin
    if RAW_ALLOWED_ORIGINS.strip() == '*':
        CORS(app, resources={r"/*": {"origins": "*"}}, supports_credentials=False)
    else:
        origin_list = [o.strip() for o in RAW_ALLOWED_ORIGINS.split(',') if o.strip()]
        CORS(app, resources={r"/*": {"origins": origin_list}}, supports_credentials=False)

def _parse_allowed():
    if RAW_ALLOWED_ORIGINS.strip() == '*':
        return '*'
    return {o.strip() for o in RAW_ALLOWED_ORIGINS.split(',') if o.strip()}

_ALLOWED_SET = _parse_allowed()
print(f"[CORS] ALLOW_ORIGINS={RAW_ALLOWED_ORIGINS}")

# Global OPTIONS preflight handler (catch-all) with explicit header echoing
@app.route('/', defaults={'path': ''}, methods=['OPTIONS'])
@app.route('/<path:path>', methods=['OPTIONS'])
def any_options(path):  # noqa: D401
    origin = request.headers.get('Origin')
    req_headers = request.headers.get('Access-Control-Request-Headers', '')
    resp = jsonify({})
    resp.status_code = 204
    # Manually add headers (after_request will still run but we ensure here too)
    allowed = False
    if _ALLOWED_SET == '*' or (origin and origin in _ALLOWED_SET):
        if origin:
            resp.headers['Access-Control-Allow-Origin'] = origin if _ALLOWED_SET != '*' else origin
        else:
            resp.headers['Access-Control-Allow-Origin'] = '*' if _ALLOWED_SET == '*' else ''
        allowed = True
    resp.headers['Access-Control-Allow-Methods'] = 'GET, POST, OPTIONS'
    # Echo requested headers plus our defaults
    base_headers = ['Content-Type', 'Authorization']
    dynamic = [h.strip() for h in req_headers.split(',') if h.strip()]
    merged = base_headers + [h for h in dynamic if h.lower() not in {bh.lower() for bh in base_headers}]
    resp.headers['Access-Control-Allow-Headers'] = ', '.join(merged)
    resp.headers['Access-Control-Max-Age'] = '600'
    resp.headers['Vary'] = 'Origin'
    print(f"[CORS][OPTIONS] path=/{path} origin={origin} allowed={allowed} req_headers={req_headers}")
    return resp
...
def _add_cors_headers(resp):
    origin = request.headers.get('Origin')
    if _ALLOWED_SET == '*':
        # Reflect requesting origin if provided, else *
        resp.headers['Access-Control-Allow-Origin'] = origin or '*'
    else:
        if origin in _ALLOWED_SET:
            resp.headers['Access-Control-Allow-Origin'] = origin
    resp.headers['Access-Control-Allow-Methods'] = 'GET, POST, OPTIONS'
    # If preflight already set a richer header list keep it; else set default
    if 'Access-Control-Allow-Headers' not in resp.headers:
        resp.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization'
    # Always inform caches vary by Origin
    resp.headers['Vary'] = 'Origin'
    return resp
IS_DEPLOY = os.getenv("DEPLOY") == "1"  # set DEPLOY=1 on Render to disable server webcam stream
IMGSZ = int(os.getenv("IMGSZ", "640"))  # configurable inference size (smaller = faster)
...
def _warmup_model():
    try:
        ...
        model.predict(dummy, imgsz=IMGSZ, conf=0.4, verbose=False)
...
# Always-available basic health (overrides earlier if exists)
...
def predict():
    ...
    # ใช้ logic เดียวกับหน้า index (group ต่อผลไม้) เพื่อให้โครงสร้างผลลัพธ์เหมือนกัน
    # Raw detection boxes (like original stream) for client overlay
    raw_boxes = []
    for det in detections:
        cls_id = int(det.cls[0])
        class_name = class_names.get(cls_id, f"Unknown_{cls_id}")
        conf_val = float(det.conf[0])
        box_vals = [int(x) for x in det.xyxy[0]]
        raw_boxes.append({
            'class': class_name,
    ...
    grouped_summary = build_frame_summary(detections, class_names)
    grouped_summary['raw'] = raw_boxes  # attach raw boxes for drawing client-side
    return jsonify(grouped_summary)
